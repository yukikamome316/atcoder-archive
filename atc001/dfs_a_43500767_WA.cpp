 ...
 * Submission URL: https://atcoder.jp/contests/atc001/submissions/43500767
 * Submitted at: 2023-07-12 14:33:15
...
bool isValidPos(pair<int, int> pos, int H, int W) {
...
bool dfs(pair<int, int> pos, pair<int, int> gpos, vector<vector<bool>>& visited, vector<string>& fi, int H, int W) {  
  ...
  rep(dir, 4) {
    ...
    if (isValidPos(new_pos, H, W) 
      && !visited[new_pos.first][new_pos.second] 
      && fi[new_pos.first][new_pos.second] != '#') {
