 ...
 * Submission URL: https://atcoder.jp/contests/dp/submissions/43471559
 * Submitted at: 2023-07-11 10:17:31
...
int main() {
  ...
  while (!(current.first == -1 && current.second == -1)) {
    if (s[current.first] == t[current.second]) {
      result.insert(result.begin(), s[current.first]);
    }
